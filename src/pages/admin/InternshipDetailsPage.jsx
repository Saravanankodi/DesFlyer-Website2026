...
import { useInternshipOpenings, openingsApi } from '../../store/openingsStore'
...
export default function InternshipDetailsPage() {
  ...
  function handleSave(values) {
    if (editing) {
      openingsApi.updateInternship(editing.id, values)
    } else {
      openingsApi.addInternship(values)
    }
    setModalOpen(false)
  }
  function handleDelete(id) {
    if (!confirm('Delete this internship opening? This will remove it from the website immediately.')) return
    openingsApi.deleteInternship(id)
  }

  return (
    <div className="p-6 lg:p-10">
      <div className="flex items-center justify-between mb-2">
        <h1 className="font-display font-bold text-2xl lg:text-3xl text-[var(--fg)]">Internship Details</h1>
        <Button onClick={openCreate} className="px-5 py-2.5">
          <FiPlus /> Add Internship
        </Button>
      </div>
      <p className="text-sm text-[var(--fg)]/50 mb-8">
        Changes here sync live to the public Internship page — this is a real shared store, not a mock.
      </p>
        ...
        <AnimatePresence>
          {openings.map((item) => (
            ...
            >
              ...
              <p className="text-xs text-[var(--fg)]/50 mt-1">{item.department}</p>
              <div className="flex flex-wrap gap-x-4 gap-y-1.5 mt-4 text-xs font-mono text-[var(--fg)]/50">
                <span className="flex items-center gap-1.5"><FiClock size={12} /> {item.duration}</span>
